...
object ComplexVROrdering extends Ordering[VaryRowComplex] {
  ...
  override def compare(x: VaryRowComplex, y: VaryRowComplex): Int = {
    if (x.vs.isEmpty) {
      return 0
    }
    x.vs.head match {
      case Tools.sortKeyType.stringKeyType | Tools.sortKeyType.stringLowerCaseKeyType => {
        // toLowerCase has already been applied
  ...
  def distance(
    ...
    x.vs.head match {
      case Tools.sortKeyType.stringKeyType | Tools.sortKeyType.stringLowerCaseKeyType => {
        ...
        val newThis = VaryRowComplex(x.v1.tail, x.v2, x.v3, x.vs.tail, x.content)
        val newThat = VaryRowComplex(y.v1.tail, y.v2, y.v3, y.vs.tail, y.content)
        ComplexVROrdering.distance(newThis, newThat, cmp.map(BigDecimal(_)) ::: prevDistance)
      }
      case Tools.sortKeyType.stringNegKeyType | Tools.sortKeyType.stringLowerCaseNegKeyType => {
        // toLowerCase has already been applied
